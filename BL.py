...
def objective2(X, data, ret, vol):
    cov_matrix_d_sample = vol

    port_volatility = np.sqrt(np.dot(X.T, np.dot(cov_matrix_d_sample, X)))
    
    port_return = ret.dot(X)
...
def optimizer2(weights, data, ret, vol):
    cons = ({'type': 'eq', 'fun': lambda weights: np.sum(weights) -1  })
    bnds = [(0, 1)] * len(weights)    # https://stackoverflow.com/questions/29150064/python-scipy-indexerror-the-length-of-bounds-is-not-compatible-with-that-of-x0
    best_mix = spo.minimize(objective2, weights, args=(data, ret, vol), method='SLSQP', bounds = bnds, constraints = cons, options={'disp':False})
...
w_mvo2 = optimizer2(w,data,ER,cov_est)

agg_weights = pd.DataFrame([w,w_est,w_mvo,w_mvo2],columns=stocks,\
                   index=['Market cap weights','Black Litterman weights',\
                          'Traditional MVO','MVO with BL generated ret & vol'])
